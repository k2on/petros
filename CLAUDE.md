...
- **The NDK is x86_64-only.** `nix develop .#android` resolves on an ARM Linux
  box and then its `clang` cannot execute — binfmt hands it to qemu, which has
  no x86-64 loader. Build Android on x86_64 or in CI. iOS needs Xcode, so it is
  a macOS runner. `.github/workflows/expo.yml` does both.
