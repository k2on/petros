...
use petros::{ActorId, AutoCtx, Client, ClientMsg, Entry, Id, Mutation, Seq, Server, ServerMsg};
...
fn hello_since_returns_exactly_the_missing_entries() {
    let mut server = Server::<Todo>::open(petros::open_memory().unwrap()).unwrap();
...
fn a_full_batch_asks_the_client_to_come_back_for_more() {
    let mut client =
        Client::<Todo>::open(petros::open_memory().unwrap(), "alice", AutoCtx::seeded(1)).unwrap();
    client
        .recv(Down::Batch {
            entries: log(3),
            has_more: true,
...
fn cursor_survives_restart() {
    ...
    let pending_id = {
        let mut client = Client::<Todo>::open(
...
fn fill_auto_values_are_frozen() {
    ...
    entry.seq = Some(1);
    let mut replica = Client::<Todo>::open(
        petros::open_memory().unwrap(),
        "bob",
        AutoCtx::seeded(123_456),
    )
    .unwrap();
...
fn replay_into(actor: &str, seed: u64, entries: Vec<Entry<TodoMutation>>) -> Client<Todo> {
    let mut c =
        Client::<Todo>::open(petros::open_memory().unwrap(), actor, AutoCtx::seeded(seed)).unwrap();
