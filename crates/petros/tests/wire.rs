...
use petros::{ActorId, Id, ServerMsg};
...
fn old_wire_bytes_still_deserialize() {
    let msg: ServerMsg<TodoMutation> = petros::decode(&unhex(FIXTURE)).expect("decode fixture");
...
fn a_field_added_with_serde_default_reads_old_bytes() {
    let old = petros::encode(&Before::Add {
        id: Id::from_u128(9),
        text: "written before the field existed".into(),
    })
    .unwrap();

    let new: After = petros::decode(&old).expect("decode old bytes with the new type");
    assert_eq!(
        new,
        After::Add {
            id: Id::from_u128(9),
